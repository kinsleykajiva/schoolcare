...
(function( $ ){
	$.fn.datetimepicker = function( opt ){
		...
		let options = ($.isPlainObject(opt)||!opt)?$.extend({},default_options,opt):$.extend({},default_options);
		let createDateTimePicker = function( input ){
			let datetimepicker = $('<div class="xdsoft_datetimepicker"></div>'),
					...
					switch(true){
						...
						case ($.type(options.mask) == 'string'):
							...
							input.on('keydown.xdsoft',function( event ){
								let val = this.value, key = event.which;
								switch(true){
									case ((key>=48&&key<=57)||(key>=96&&key<=105))||(key==BACKSPACE||key==DEL):
										let pos = getCaretPos (this),
									...
									case [ENTER,TAB].indexOf(options.mask,key)!==-1:
										const elementSelector = "input:visible,textarea:visible";
			...
			(function(){
				...
				scroller.on('mousedown.xdsoft',function( event ){
					...
					$('body').on('mousemove.xdsoft',move = function(event){
						let offset = event.pageY - pageY + top;
						if( offset<0 )
							offset = 0;
				...
				datetimepicker.on('scroll.timebox',function( event,offset ){
					if( !options.timepickerScrollbar )
						return;
					let sbh = scrollbar.height () - scroller[0].offsetHeight,
			...
			datetimepicker.on('scroll.scrollbar',function(event,offset){
				let sbh = scrollbar[0].offsetHeight - scroller[0].offsetHeight,
					percent = offset / sbh;
				pheight = timeboxparent[0].offsetHeight-2,
					height = timebox[0].offsetHeight;
			...
			timepicker.find('.xdsoft_prev,.xdsoft_next').on('mousedown.xdsoft',function(){
				let $this = $ (this), timer = 0, stop = false, period = 110;
				(function(v){
					var pheight = timeboxparent[0].offsetHeight-2,
						height = timebox[0].offsetHeight,
			...
			datetimepicker.on('change.xdsoft',function(){
				let j;
				let xd = $ (this).data ('xdsoft_datetime'),
					table = '';
				let start = new Date (xd.currentTime.getFullYear (), xd.currentTime.getMonth (), 1);
				while( start.getDay()!=options.dayOfWeekStart )
					start.setDate(start.getDate()-1);
				let i = 0, today = new Date;
				...
				if( !options.allowTimes || !$.isArray(options.allowTimes) || !options.allowTimes.length ){
					for(i = 0; i<(options.hours12?12:24); i++){
					...
					for(i = 0; i<options.allowTimes.length; i++){
			...
			datetimepicker.on('open.xdsoft',function(){
				if( timebox.find('.xdsoft_current').length ){
					let pheight = timeboxparent[0].offsetHeight - 2,
						height = timebox[0].offsetHeight,
			...
			calendar.on('mousedown.xdsoft','td',function(){
				if($(this).hasClass('xdsoft_disabled'))
					return false;
				let ct = datetimepicker.data ('xdsoft_datetime').currentTime;
			...
			let _xdsoft_datetime = new xdsoft_datetime, timer = 0, timer1 = 0;
		...
		let destroyDateTimePicker = function (input) {
			let datetimepicker = input.data ('xdsoft_datetimepicker');
			if (datetimepicker) {
				let _xdsoft_datetime = datetimepicker.data ('xdsoft_datetime');
